...
host job claiming" out of scope. As a result every pending `review_runs`
row sat unclaimed forever: nothing assigned one to a host, tracked a
lease, sent a heartbeat, or reconciled a lost host. GEN-414 closes that
gap, adding the host-facing claim layer on top of the existing engine
...
**`review_runs.claimed_by_host_id` is a permanent audit field, not a
lease that gets released.** This is a deliberate difference from `issues.
active_host_id`/`active_run_id`, which *do* get cleared on release because
...
`running` is the sole liveness signal, and `claimed_by_host_id` simply
...
... skip locked`**, the same pattern `reconcile_offline_host_runs`
(GEN-412) already uses for stale hosts. A second concurrent caller skips a
row already being claimed and either wins a different eligible one or finds
none — this is what makes "two hosts cannot claim the same review job" a
database guarantee rather than an application-level race the caller has to
avoid.

**Every recovery path — the claim route's post-claim rollback, host
ban/delete release, and offline-host reconciliation — reuses
...
extension of `reconcile_offline_host_runs`.** The two use structurally
...
Reconciliation excludes banned hosts (mirroring the issues reconciler), so
— exactly as `issues` already requires via `requeue_host_active_issues` —
`ban_host` and `delete_host` both gained a parallel call to a new
`requeue_host_active_review_runs`, which releases a host's claimed
...
**A review run's staleness is judged two ways**, not just by host
heartbeat: `coalesce(hosts.last_seen_at, hosts.offline_since_at)` (the
host is gone) **or** `coalesce(review_runs.heartbeat_at, review_runs.
started_at)` (the run itself stopped progressing on an otherwise-healthy
host). The latter only matters once a real, potentially long-running
...
**Implementation priority is enforced by claim ordering on the host CLI,
not by a combined atomic endpoint.** Every poll tick, `apps/gentic/src/
host.ts` always calls `claimNextQueuedIssue` before ever attempting
`claimReviewRun`, and only attempts the latter if the former returned
nothing. Because `listRunningTaskCounts` in
`packages/services/src/hosts/shared.ts` now unions counts from both
`issues` and `review_runs` into one map,
implementation and review jobs share a single capacity pool per host: a
...
**The host CLI's `processReviewRun` is a deliberate, temporary stub.**
...
- New columns `review_runs.claimed_by_host_id`/`heartbeat_at`, a new RPC
  ...
  calls it yet.
- `apps/gentic/src/host.ts` gained a second concurrency-tracking map
  ...
  implemented by GEN-413 — reaches a host holding the claim).
- `ban_host` and `delete_host` (GEN-412) were extended with one call
  each to the new `requeue_host_active_review_runs`.
