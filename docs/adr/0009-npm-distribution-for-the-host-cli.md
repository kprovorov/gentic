# 9. npm distribution for the host CLI
...
falls back to `require.resolve` plus `node <entry>`, which is how the host
...
- The npm install requires Node.js 20.19+ on the host, which the
