...
description: "Use issue relations in Gentic to declare dependencies between tasks. A blocked issue is skipped by the worker until its blocker is resolved."
---

Issue relations let you describe dependencies between coding tasks so the agent worker executes them in the right order. When one issue blocks another, Gentic automatically holds the blocked issue in the queue until its dependency is resolved — no manual coordination required.

## What issue relations are

A relation connects two issues with a directional dependency: issue A **blocks** issue B. The agent worker respects this constraint at the moment it polls for work. If issue B is in the `Todo` queue but it is blocked by issue A, the worker skips issue B and moves on. Once issue A reaches a resolved state such as `merged`, `completed`, or `cancelled`, issue B becomes eligible and the worker picks it up on the next poll cycle.
...
- A blocker is considered resolved when its work has landed or ended, such as `merged`, `completed`, or `cancelled`.
...
Click **Add** to save. The relation appears in the **Relations** card immediately, and the issue's **Blocked** badge updates after the data refreshes.
...
|-----------|---------|--------|
| **This issue is blocked by** issue A | Issue A must resolve before this issue runs | Worker skips this issue until A is `merged`, `completed`, or `cancelled` |
| **This issue blocks** issue B | This issue must resolve before issue B runs | Worker skips issue B until this issue is `merged`, `completed`, or `cancelled` |
...
<Note>
  Issue relations only affect the **agent worker's** execution order. You can still manually change an issue's status or retry the agent regardless of its blocked state. The **Blocked** badge is a reminder, not a hard lock on your own actions.
</Note>
