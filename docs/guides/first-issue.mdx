...
After the status moves to **Todo**, the issue transitions to **Queued** as soon as a worker claims it and then to **In Progress** once the agent session starts. The **Transcript** panel on the issue detail page streams the agent's output in real time — you can watch it read files, reason about the change, and write code.
