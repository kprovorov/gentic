...
| **Tools**    | Install and authentication state of the external CLIs this worker shells out to: `GitHub CLI` (`gh`) plus only the selected agent provider CLIs (`Claude` and/or `Codex`). Each row shows a colored icon (✓ green when authenticated, ⚠ yellow when installed but not authenticated, ✗ red when missing), the status text, and the detected version. |

### Why the tools check matters

Every issue run shells out to `gh` to interact with GitHub, and to either `claude` or `codex` depending on the issue's agent provider. The worker only claims issues for providers selected during `gentic auth login` or through `AGENT_PROVIDERS`, so `gentic status` only checks those provider CLIs. Checking `gentic status` after installing the worker — and any time you rotate credentials — surfaces those problems before you queue a real issue.
