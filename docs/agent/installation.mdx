...
<Warning>
  ...
  com.apple.quarantine ./gentic` from the terminal. The Homebrew formula handles
  this automatically.
</Warning>
...
During `gentic auth login`, choose which coding agent(s) this worker will run. The prompt defaults to **Claude Code**. Gentic uses that selection to decide which provider checks to run and which issues this worker can claim; unselected providers are skipped entirely.
