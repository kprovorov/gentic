...
<Steps>
  ...
  <Step title="Upgrade later">
    ...
    You can also run this upgrade from the web app: **Settings → Workspace → Hosts → Update tools** queues it on the host, which installs the new version, finishes its active tasks, and restarts itself. See [Update tools on a host](/web/hosts#update-tools-on-a-host).
