...
description: "Deploy the Gentic agent worker from npm, a Linux package, the prebuilt tarball, or from source with Node.js. Includes Git and credentials setup."
---
...
All methods produce the same `gentic` CLI. After installing, follow the [Git](#git-authentication-setup) setup below, then configure credentials only for the agent providers this worker will run.
...
The install brings the ACP agent runtimes the worker spawns, so — as with every other option — the only things left are [Git access](#git-authentication-setup) and the [agent CLIs and credentials](#agent-credentials-setup). Run `gentic doctor` to see what is still missing.
...
Gentic workers can claim issues for any supported agent provider. Install and authenticate both Claude Code and Codex in the worker environment before starting the worker.
...
<Tip>
  ...
  this by setting the variable in the worker's environment before starting the
  service.
</Tip>
