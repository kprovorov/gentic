...
1. **Gentic auth** — prompts for `GENTIC_API_URL` and `GENTIC_API_KEY`. This is required.
...
```bash
gentic auth login --api-url https://app.gentic.chat/api/v1 --api-key <your-key>
gentic run
```
...
Run `gentic auth login` to start the interactive prompt. It asks for your API URL and API key, then saves them to the config file. The worker always supports both Claude Code and Codex.
