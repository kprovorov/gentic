...
<Note>
  `GENTIC_API_KEY` must be a user-scoped API key accepted by the Gentic deployment. The worker only receives issues belonging to that user. Ask the deployment administrator how user API keys are issued; the current Gentic web settings page does not create them.
</Note>
