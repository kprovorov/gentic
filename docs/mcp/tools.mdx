...
Returns full details for a single issue, including its current status, priority, prompt, and agent configuration.
...
Updates the title, prompt, agent provider, issue model, type, and priority for an existing issue. Use this to refine the agent's instructions before queuing a run. `title`, `agent_provider`, `type`, and `priority` are required; `prompt` and `issue_model` are optional.
