...
<ParamField body="host" type="Any host | <host name>">
  Which [host](/web/hosts) runs the issue. **Any host** (the default) leaves it on the shared queue, where the first eligible host to poll claims it. Choosing a host **pins** the issue: only that host will ever claim it, so it waits while that host is offline, busy, or banned rather than moving to another machine. The picker lists your enrolled hosts with their current state and leaves out banned ones. You can change the pin later on the edit page; the issue detail page shows it under **Host**.
</ParamField>
...
Gentic merges with the first method your repository still allows, preferring **squash**, then **merge commit**, then **rebase**, and leaves the commit message to your repository's own defaults. The merge is pinned to the exact commit the approval was checked against, so a push that lands between your click and the merge is rejected rather than merged unreviewed.

Gentic re-checks the pull request's live state before merging, so if the approval was dismissed or the PR was closed since the page last refreshed, the button reports that instead of merging. Merging requires the GitHub App's **Contents** permission to be **Read & write** — see [GitHub Integration](/web/github-integration).
...
Open the issue detail page and click **Edit**. You can update the **Title**, **Body**, **Agent**, **Type**, **Host**, and **Create PR automatically** fields. Once a pull request is attached, the publishing preference becomes read-only historical data. The **Project** field cannot be changed after creation.

Changing **Host** takes effect at the next claim: an issue already running somewhere keeps running there, and the new pin applies the next time it is queued. An issue pinned to a host that has since been banned stays editable, so you can move it to **Any host** or another host without unbanning first.

## Reset an Issue

Resetting clears the agent session and re-queues the issue as if it were starting fresh, on the agent and model it is already set to. Open the issue, click the **⋮** menu beside the title, choose **Reset**, and confirm.

Reset is offered on any issue that has left **Draft**. A draft has nothing to reset — move it to **Todo** to start it. A [Spec](#spec-issues) never shows the action, because no agent runs it.
