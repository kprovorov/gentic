...
<Steps>
  ...
  <Step title="Subscribe to required webhook events">
    In the GitHub App's **Subscribe to events** section, enable **Pull requests**, **Pull request reviews**, **Check runs**, **Check suites**, and **Workflow runs**. The app needs at least read access to the **Checks** permission to receive check events. Check and workflow-run events are required for issues in `testing` to move automatically to `ready-for-review` or `tests-failed` after CI finishes.
  </Step>
