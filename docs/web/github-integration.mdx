...
<Steps>
  ...
  <Step title="Set the repository permissions">
    ...
    | **Contents** | Read-only | Resolve commits and branches referenced by PR and check events. |
    | **Checks** | Read-only | Receive check-run and check-suite events. |
    | **Metadata** | Read-only | Mandatory for every GitHub App. |

    <Warning>
      Raising **Pull requests** to **Read & write** on an app that is already installed does **not** take effect on its own. GitHub records it as a pending permission request that the account or organization owner must accept at **Settings → Applications → Installed GitHub Apps → Gentic → Review request**. Until it is accepted, the installation keeps its old read-only access and every Automatic Review publish fails with `403 Resource not accessible by integration`.
    </Warning>
  </Step>
