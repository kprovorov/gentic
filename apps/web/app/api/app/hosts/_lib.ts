...
export const hostMutationNoStoreHeaders = {
  ...
  return async function handler(
    ...
    if (!context) {
      return Response.json(
        ...
        { status: 401, headers: hostMutationNoStoreHeaders }
      )
    }
    ...
    try {
      return Response.json(
        ...
        { headers: hostMutationNoStoreHeaders }
      )
    } catch (error) {
      return handleHostMutationError(error)
    }
  }
}

function handleHostMutationError(error: unknown): Response {
  if (error instanceof ServiceError) {
    if (error.code === "internal") {
      console.error("[hosts-api] request failed:", error)
    }
  ...
  if (error instanceof SyntaxError || error instanceof z.ZodError) {
    return Response.json(
      ...
      { status: 400, headers: hostMutationNoStoreHeaders }
    )
  }

  console.error("[hosts-api] request failed:", error)
  return Response.json(
    { error: { code: "internal", message: "Unable to update host" } },
    { status: 500, headers: hostMutationNoStoreHeaders }
  )
}
