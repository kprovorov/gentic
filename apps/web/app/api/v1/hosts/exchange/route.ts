...
import { exchangeHostEnrollmentCode } from "@gentic/services/hosts"
...
export const POST = createHostExchangeHandler({
  createSupabase: createServiceClient,
  exchange: exchangeHostEnrollmentCode,
})

export function createHostExchangeHandler(deps: {
  createSupabase: typeof createServiceClient
  exchange: typeof exchangeHostEnrollmentCode
}) {
  return async function POST(request: Request) {
    try {
      ...
      return Response.json({
        ...
        host: {
          id: result.host.id,
          display_name: result.host.display_name,
          setup_state: result.host.setup_state,
        },
