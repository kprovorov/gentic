...
test("reloaded config applies runtime settings and keeps host connection settings", () => {
  const current = testConfig()
  const next: Config = {
    ...current,
    GENTIC_HOST_ID: "host-2",
    GENTIC_HOST_CREDENTIAL: "new-key",
  ...
  assert.equal(current.GENTIC_HOST_ID, "host-1")
  assert.equal(current.GENTIC_HOST_CREDENTIAL, "test-key")
...
test("fresh and resumed runs both hand the agent authenticated Gentic MCP access", async () => {
  for (const sessionId of [null, "existing-session"]) {
    await withHarness(async ({ config, issue, api, deps }) => {
      ...
      // No host-side preflight: nothing probes the MCP endpoint before the
      // session starts.
      assert.deepEqual(api.finishedStatuses, ["waiting-for-input"])
    })
  }
...
test("host loop sends heartbeat every 30s and checks control every 10s", async () => {
  const clock = new FakeClock()
  await withHarness(
    async ({ config, api, deps }) => {
      config.POLL_INTERVAL_MS = CONTROL_INTERVAL_MS
      api.controlResponse = () => ({
        host: { banned: clock.elapsedMs >= 61_000 },
        runs: [],
      })
...
test("host loop reloads config on SIGHUP without stopping", async () => {
  const clock = new FakeClock()
  await withHarness(
    async ({ config, api, deps }) => {
      config.POLL_INTERVAL_MS = CONTROL_INTERVAL_MS
      api.controlResponse = () => ({
        host: { banned: clock.elapsedMs >= 21_000 },
        runs: [],
      })
      ...
      await runHostLoop(api, config, hostDeps)
...
test("host loop marks graceful shutdown offline immediately", async () => {
  const clock = new FakeClock()
  await withHarness(async ({ config, api, deps }) => {
    const stop = () => process.emit("SIGTERM")
    clock.onSleep = stop
    api.controlResponse = () => ({ host: { banned: false }, runs: [] })
...
test("host loop waits for signal-triggered offline update", async () => {
  const clock = new FakeClock()
  await withHarness(
    async ({ config, api, deps }) => {
      ...
      const run = runHostLoop(api, config, loopDeps(deps, clock, api)).then(
        () => {
          completed = true
        }
      )
...
test("host loop survives transient heartbeat and control failures", async () => {
  const clock = new FakeClock()
  await withHarness(
    async ({ config, api, deps }) => {
      config.POLL_INTERVAL_MS = CONTROL_INTERVAL_MS
      ...
      api.controlResponse = () => ({
        host: { banned: clock.elapsedMs >= 31_000 },
        runs: [],
      })
...
test("host heartbeat survives transient provider check failures", async () => {
  const clock = new FakeClock()
  await withHarness(
    async ({ config, api, deps }) => {
      config.POLL_INTERVAL_MS = CONTROL_INTERVAL_MS
...
test("host control ban aborts active sessions without recording failures", async () => {
  const clock = new FakeClock()
  await withHarness(
...
test("a skill install runs alongside issue work without interrupting it", async () => {
  ...
  await withHarness(
    async ({ config, issue, api, deps }) => {
      ...
      // actually run. The host starts `processIssue` without awaiting it, and
      ...
      const hostDeps = loopDeps(deps, clock, api)
      hostDeps.createSkillInstallRunner = (api_) =>
...
test("host control invalidates active runs and cancels their sessions", async () => {
  const clock = new FakeClock()
  await withHarness(
    async ({ config, issue, api, deps }) => {
      config.MAX_CONCURRENT_ISSUES = 1
      ...
      let sessionEntered = false
      api.controlResponse = () => ({
        host: { banned: sessionEntered && clock.elapsedMs >= 20_000 },
        runs:
...
test("host does not discover agent-created pull requests from the checkout", async () => {
  await withHarness(async ({ config, issue, api, deps }) => {
    issue.createPrAutomatically = true
    api.hasChanges = true
...
test("host restarts do not loop when the active run already has an automatic request", async () => {
  await withHarness(async ({ config, issue, api, deps }) => {
    issue.createPrAutomatically = true
...
async function withHarness(
  ...
  const workdir = await mkdtemp(join(tmpdir(), "gentic-host-test-"))
  try {
...
function fakeDeps(
  ...
  return {
    ...
    async cloneRepoAtSha(options) {
      ...
      // A real directory, so the cleanup-on-exit assertions in host.test.ts
...
class FakeApi implements AgentApi {
  ...
  readonly heartbeats: HostHeartbeatTelemetry[] = []
  ...
  readonly reviewRunLogs: { reviewRunId: string; input: ReviewRunLogInput }[] =
    []
  readonly pendingSkillInstalls: HostSkillInstallCommand[] = []
  ...
  // `review_runs` defaults to `[]` in `fetchHostControl` below, so
  // existing call sites that only cover implementation-issue control state
  // don't all need updating for GEN-414's new field.
  controlResponse: () => Omit<HostControlResponse, "review_runs"> &
    Partial<Pick<HostControlResponse, "review_runs">> = () => ({
    host: { banned: true },
    runs: [],
  })
  ...
  async sendHeartbeat(telemetry: HostHeartbeatTelemetry): Promise<void> {
  ...
  async fetchHostControl(): Promise<HostControlResponse> {
  ...
  async claimSkillInstall(): Promise<HostSkillInstallCommand | null> {
