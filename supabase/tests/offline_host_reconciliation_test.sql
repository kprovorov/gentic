...
SELECT is(
  ...
  '30 seconds',
  'offline-host reconciliation runs every 30 seconds'
);
...
SELECT is(
  ...
  'select public.reconcile_offline_host_runs();',
...
INSERT INTO public.issues (
  ...
  (
    ...
    '10000000-0000-4000-8000-000000000041',
    'Second task on the stale host',
    'Concurrent assignment body',
    'queued',
    4,
...
SELECT is(
  public.reconcile_offline_host_runs('2026-07-29T11:59:59.999Z'),
  0,
...
SELECT is(
  public.reconcile_offline_host_runs('2026-07-29T12:00:00Z'),
  3,
...
SELECT is(
  ...
  'run-failed',
  'a stale host active task becomes run-failed'
);
...
SELECT is(
  ...
  'run-failed',
  'one invocation fails every active task assigned to the stale host'
);
...
SELECT is(
  ...
  2,
  'one invocation records one event for each stale host task'
);
...
SELECT is(
  ...
  null,
  'the stale host lease is cleared'
);
...
SELECT is(
  ...
  'assigned_host_offline',
  'the failure event records a structured offline reason'
);

SELECT is(
  (
    SELECT payload -> 'reason' ->> 'host_id'
      FROM public.issue_events
     WHERE issue_id = '50000000-0000-4000-8000-000000000041'
       AND type = 'run_failed'
  ),
  '40000000-0000-4000-8000-000000000041',
  'the structured reason preserves the stale host id'
);
...
SELECT is(
  ...
  'reconciliation leaves the stale host record offline'
);
...
SELECT is(
  ...
  'run-failed',
  'a late host heartbeat does not resume the failed task'
);
...
SELECT is(
  ...
  null,
  'a late host heartbeat does not restore the old run id'
);
...
SELECT is(
  (
    SELECT count(*)::integer
      FROM public.issues
     WHERE active_host_id = '40000000-0000-4000-8000-000000000041'
       AND active_run_id = '60000000-0000-4000-8000-000000000041'
  ),
  0,
  'host control cannot return the invalidated local session'
);
...
SELECT is(
  ...
  null,
  'manual retry leaves the failed host lease cleared'
);
...
SELECT is(
  ...
  null,
  'the scheduler does not mutate a host explicitly marked offline'
);
