-- Review job offline-host reconciliation (GEN-414 / ADR-0005):
-- `reconcile_offline_review_runs` fails a claimed review run whose host
-- has gone dark, or whose reviewer session itself stopped heartbeating on
-- an otherwise-live host, by reusing `fail_review_run` — so it inherits
...
INSERT INTO public.issues (id, project_id, title, body, status, number, agent_provider) VALUES
  ('d0000000-0000-4000-8000-000000000011', 'd0000000-0000-4000-8000-000000000001', 'Stale host', 'Body', 'ready-for-review', 1, 'claude_code'),
  ('d0000000-0000-4000-8000-000000000012', 'd0000000-0000-4000-8000-000000000001', 'Fresh host', 'Body', 'ready-for-review', 2, 'claude_code'),
  ...
  ('d0000000-0000-4000-8000-000000000016', 'd0000000-0000-4000-8000-000000000001', 'Banned host holds the claim', 'Body', 'ready-for-review', 6, 'claude_code');
...
INSERT INTO public.hosts (id, user_id, display_name, credential_hash, last_seen_at, updated_at) VALUES
  ('d0000000-0000-4000-8000-000000000031', 'recon_test_user', 'Stale Host', repeat('1', 64), '2026-08-19T11:54:00Z', '2026-08-19T11:54:00Z'),
  ('d0000000-0000-4000-8000-000000000032', 'recon_test_user', 'Fresh Host', repeat('2', 64), '2026-08-19T11:59:00Z', '2026-08-19T11:59:00Z'),
  ('d0000000-0000-4000-8000-000000000033', 'recon_test_user', 'Stale Heartbeat Host', repeat('3', 64), '2026-08-19T11:59:00Z', '2026-08-19T11:59:00Z'),
  ('d0000000-0000-4000-8000-000000000034', 'recon_test_user', 'Two Strikes Host', repeat('4', 64), '2026-08-19T11:54:00Z', '2026-08-19T11:54:00Z');

INSERT INTO public.hosts (id, user_id, display_name, credential_hash, last_seen_at, updated_at) VALUES
  ('d0000000-0000-4000-8000-000000000036', 'recon_test_user', 'Banned Host', repeat('6', 64), '2026-08-19T11:54:00Z', '2026-08-19T11:54:00Z');
...
-- banned host could never have claimed it in the first place.
...
-- Issue 3's run is claimed by a *fresh* host, but the run itself stopped
-- heartbeating — the reconciler must catch this even though the host
...
SELECT is(
  ...
  3,
  'one pass reconciles the stale-host, stale-heartbeat, and two-strikes-host runs (not the fresh or banned-host ones)'
);

SELECT is(
  (SELECT status FROM public.review_runs WHERE id = (SELECT review_run_id FROM claim1)),
  'failed',
  'the stale host''s run is failed'
);
SELECT is(
...
SELECT is(
  ...
  1,
  'the stale-host run automatically retries with a fresh pending run in the same cycle'
);
SELECT is(
...
SELECT is(
  ...
  'running',
  'a run held by a host within the 5-minute window is untouched'
);

SELECT is(
  (SELECT status FROM public.review_runs WHERE id = (SELECT review_run_id FROM claim3)),
  'failed',
  'a stale run heartbeat fails the run even though the host itself looks healthy'
);

SELECT is(
  (SELECT status FROM public.review_runs WHERE id = (SELECT review_run_id FROM claim4)),
  'failed',
  'the two-strikes host''s first failure is reconciled normally'
);
...
-- with no host affinity, and pull/5's still-pending run is older than any
-- retry created above -- without this, the "same host" reclaim below would
...
SELECT is(
  ...
  'the same host re-claims its own automatic retry'
);
...
-- The fresh host, and the run it's holding, were only ever given a
...
-- alongside the two-strikes host's run.
UPDATE public.hosts
...
SELECT is(
  ...
  1,
  'a second stale pass reconciles only the two-strikes host''s retried run'
);
SELECT is(
...
SELECT is(
  ...
  'pending',
  'a never-claimed run is untouched by reconciliation regardless of any host''s staleness'
);

SELECT is(
  (SELECT status FROM public.review_runs WHERE id = (SELECT review_run_id FROM claim6)),
  'running',
  'a banned host''s running review run is left alone by this reconciler (released separately on ban)'
);
...
SELECT is(
  (SELECT last_seen_at FROM public.hosts WHERE id = 'd0000000-0000-4000-8000-000000000031'),
  '2026-08-19T11:54:00+00'::timestamptz,
  'reconciliation does not mutate the stale host record itself'
);

-- ---------------------------------------------------------------------
-- Restart resumes cleanly (GEN-420): a host that went offline mid-review,
...
-- pass -- the Fresh Host's still-`running` claim2 from the scenario
...
INSERT INTO public.issues (id, project_id, title, body, status, number, agent_provider) VALUES
  ('d9000000-0000-4000-8000-000000000011', 'd9000000-0000-4000-8000-000000000001', 'Restarted host', 'Body', 'ready-for-review', 1, 'claude_code');
...
INSERT INTO public.hosts (id, user_id, display_name, credential_hash, last_seen_at, updated_at) VALUES
  ('d9000000-0000-4000-8000-000000000031', 'recon_restart_user', 'Restarting Host', repeat('9', 64), '2026-08-19T13:00:00Z', '2026-08-19T13:00:00Z');
...
-- The host crashes: no further heartbeat, and it goes fully silent
...
SELECT is(
  ...
  1,
  'the crashed host''s run is reconciled'
);
...
-- The same host identity restarts (per GEN-412, `GENTIC_HOST_ID`/
...
SELECT is(
  ...
  'the restarted host successfully claims the pending run reconciliation freed for it'
);
SELECT is(
