...
INSERT INTO public.issues (
  id, project_id, title, body, status, number, active_worker_id, active_run_id
) VALUES
  (
...
SELECT is(
  (SELECT active_worker_id FROM public.issues WHERE number = 2),
  null,
  'finish-first convergence releases worker capacity'
);
SELECT is(
  (SELECT active_run_id FROM public.issues WHERE number = 1),
  null,
...
SELECT is(
  (SELECT active_worker_id FROM public.issues WHERE number = 1),
  null,
  'webhook-first convergence releases worker capacity'
);
